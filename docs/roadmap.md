...
v0.1〜v0.8 で PR レビューの基本ワークフロー（閲覧→コメント→承認→マージ）が完成する。v0.2.0 以降は「レビューする側」から「PR を作る側」のワークフローへ拡張し、さらに UX の洗練と安定性の向上を目指す。

---

### v0.2.0 — PR 作成・編集

**目的**: レビューだけでなく、PR の作成・編集もターミナルで完結させる。

現在のツールは「レビューする側」の機能に特化している。PR を作る側のワークフローを追加することで、CodeCommit 上の PR ライフサイクル全体をターミナルでカバーする。

#### 機能

| 機能 | 内容 |
|------|------|
| PR 新規作成 | ブランチ選択 → タイトル・説明入力 → PR 作成（`N` キー） |
...
```
v0.2.0 PR 作成・編集      ← 「レビューする側」から「作る側」へ拡張。PR ライフサイクル全体をカバー
 │
v0.3.0 リアクション        ← 軽量フィードバック。実装コストが低く、UX 向上効果が高い
 │
...
| `n` | 次ページ | PR 一覧 | v0.8 |
| `p` | 前ページ | PR 一覧 | v0.8 |
| `N` | PR 新規作成 | PR 一覧 | v0.2.0 |
| `E` | PR タイトル/説明編集 | PR 詳細 | v0.2.0 |
| `g` | リアクション | PR 詳細 | v0.3.0 |
...
### v0.2.0 以降（PR ライフサイクルの拡張と UX 成熟）

```
v0.2.0 PR 作成・編集    ← 「作る側」のワークフローへ拡張
 │
v0.3.0 リアクション     ← 軽量フィードバック。実装コストが低く効果が高い
 │
...
各バージョンは独立してリリース可能。v0.1.0 まででレビューワークフローが完結し、v0.2.0 以降は PR ライフサイクル全体のカバーと UX の成熟を目指す。
