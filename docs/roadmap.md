...
| `Tab` | 次のビューへ切り替え | PR 詳細 | v0.7 |
| `Shift+Tab` | 前のビューへ切り替え | PR 詳細 | v0.7 |
| `e` | コメント編集 | PR 詳細 | v0.8 |
| `d` | コメント削除 | PR 詳細 | v0.8 |
| `f` | ステータスフィルタ | PR 一覧 | v0.9 |
| `/` | 検索 | PR 一覧 | v0.9 |
| `n` | 次ページ | PR 一覧 | v0.9 |
| `p` | 前ページ | PR 一覧 | v0.9 |
...
v0.6 マージ操作      ← ワークフロー全体をターミナルで完結
 │
v0.7 コミット単位レビュー ← 大きなPRのレビュー品質を向上
 │
v0.8 コメント編集・削除 ← コメント管理の仕上げ
 │
v0.9 フィルタ・検索   ← 大量のPRを扱うチーム向けの利便性
```

各バージョンは独立してリリース可能。v0.3〜v0.6 でレビューワークフローが完結し、v0.7 でコミット単位のレビューを追加、v0.8〜v0.9 は利便性の向上にあたる。
