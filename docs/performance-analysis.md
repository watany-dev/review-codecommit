...
|--------|---|------|----------|------------|----------------|
| ★★★ | 10 | diffTextStatus デフォルト値 | 小 | 🟢 非常に低 | **即対応**。1行変更、リスクなし |
| ★★★ | 13 | TextDecoder 毎回生成 | 小 | 🟢 非常に低 | **即対応**。1行変更、リスクなし |
| ★★★ | 16 | extractAuthorName 繰返し | 小 | 🟢 非常に低 | **即対応**。キャッシュ追加のみ |
| ★★★ | 7 | header index 再計算 | 小 | 🟢 低 | **即対応**。既存 useMemo の参照に変更 |
| ★★★ | 8 | キャッシュ mutation | — | 🟢 低 | **即対応**。スプレッドコピーに変更 |
...
1. **まず #10, #13, #16, #7, #8 を対応**（リスクなし、即効性あり）
